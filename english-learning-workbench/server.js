...
async function fetchFeed(feed) {
  ...
  try {
    const response = await fetch(feed.url, {
      ...
      headers: {
        'user-agent': 'Yingxue-Ziru-Learning-Aggregator/1.0'
      }
    });
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/api/daily-content') {
      sendJson(res, await getDailyContent());
      return;
    }
    ...
    res.end(JSON.stringify({ error: 'content_aggregation_failed', message: error.message }));
  }
});

server.listen(PORT, () => {
  console.log(`英学自如聚合服务已启动：http://localhost:${PORT}`);
});
